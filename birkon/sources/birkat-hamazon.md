# Birkat HaMazon: sources and edits

Section file: `birkon/sections/birkat-hamazon.tex`

## Source

- **Text:** *Siddur Edot HaMizrach*, version "Torat Emet 357", as published by
  Sefaria (Sefaria-Export, September 2026): `Post Meal Blessing`.
- **Original source:** Torat Emet freeware (toratemetfreeware.com).
- **License:** **Public Domain** (per Sefaria's version metadata). Safe for a
  commercial print run.
- Not used: Sefaria's "Shaliehsaboo Edition" of the same siddur (its license is
  listed as "unknown").

This is a **general Edot HaMizrach base text**. The goal is the Seattle Turkish
nusach; open items are listed under "To do" below.

## Editorial decisions

- **No Shir HaMaalot / Al Naharot** before Birkat HaMazon: most Sephardic
  communities do not say them (Yosef). The section opens as the source does,
  with mayim aharonim, Lamnatzeach (Psalm 67), and Avarcha.
- **Rubrics** translated from the source's Hebrew into English.
- **With ten, add אֱלֹהֵינוּ** and **on Yom Tov, add טוֹב**: printed in square
  brackets, explained in the rubric above.
- **Whispered Amen** after Boneh Yerushalayim printed in parentheses (house style).
- **Bold opening words** kept from the source, including the bold letters of
  פּוֹתֵחַ אֶת יָדֶךָ.
- The source's `*` separators between the Harachaman lines were removed; the
  lines run on as one paragraph.

## Corrections to the source text

| Source | Printed | Where |
|---|---|---|
| וּפָּקְדֵנוּ | וּפָקְדֵנוּ | Ya'aleh VeYavo |
| יְרוּשָׁליִם | יְרוּשָׁלַיִם | Boneh Yerushalayim |
| בּאַהֲבָה | בְּאַהֲבָה | forgot Retzeh |
| ולִבְרִית | וְלִבְרִית | forgot Retzeh |
| לְזִּכָּרוֹן | לְזִכָּרוֹן | forgot Ya'aleh VeYavo |
| יחַדֵּשׁ | יְחַדֵּשׁ | Harachaman, Rosh HaShanah |
| למוֹעֲדִים ... לשָׁלוֹם | לְמוֹעֲדִים ... לְשָׁלוֹם | Harachaman, festivals |

Words the source left **unvoweled**, vowelized here:
בִּרְשׁוּתְכֶם · שָׁמַיִם · וּבִרְשׁוּתְכֶם · חַג הַשָּׁבוּעוֹת הַזֶּה · הַזִּכָּרוֹן הַזֶּה ·
שְׁמִינִי חַג עֲצֶרֶת הַזֶּה · טוֹב · מִגְדּוֹל

## Seattle Turkish nusach changes

Taken from Yosef's Turkish siddur (Birkat HaMazon, pp. 345–348).

- **Nodeh** opens with the heading (עַל אַרְצֵנוּ וְעַל נַחֲלַת אֲבוֹתֵינוּ).
- **Boneh Yerushalayim** concludes בּוֹנֵה בְּרַחֲמָיו בִּנְיַן יְרוּשָׁלַיִם עִיר הַקֹּדֶשׁ.
- **Closing verses** added after Magdil: כִּי הִשְׂבִּיעַ נֶפֶשׁ שֹׁקֵקָה … הוֹדוּ … (twice)
  … הַשָּׁמַיִם שָׁמַיִם … וְהָאָרֶץ נָתַן לִבְנֵי אָדָם (Psalms 107:9, 136:1, 115:16).
- **Oseh Shalom** stands as its own closing paragraph.
- **Ya Komimos** (Ladino) follows Birkat HaMazon, in a box, transcribed from
  the siddur (p. 348).

**Licensing:** the Ladino transliteration and any wording copied from this
siddur may be under the publisher's copyright. Confirm before selling copies.

## To do

Seen in the siddur photos but **not yet applied** (Yosef to decide):

- [ ] **First blessing:** opens with the heading (הַזָּנֵנוּ וְלֹא מִמַּעֲשֵׂנוּ …
      הַמַּעֲדִיף טוּבוֹ עָלֵינוּ), and the wording differs (בְּחֵן בְּחֶסֶד וּבְרַחֲמִים
      הוּא נוֹתֵן לֶחֶם …; concludes הַזָּן בְּרַחֲמָיו אֶת הַכֹּל). Needs a straight-on
      photo of the whole blessing.
- [ ] **Ya'aleh VeYavo festival lines:** the siddur reads
      בְּיוֹם חַג הַמַּצּוֹת הַזֶּה וּבְיוֹם (טוֹב) מִקְרָא קֹדֶשׁ הַזֶּה, with the optional
      טוֹב in parentheses; Shemini Atzeret is labeled Shemini Atzeret / Simhat Torah.
- [ ] **Hatov VeHametiv:** וְרֶוַח (the source has וְרֵיוַח).
- [ ] **Harachaman:** one line each, and includes בְּרֶוַח וְלֹא בְצִמְצוּם. Needs
      the full list (pp. 346–347).
- [ ] **The Name** is printed יְיָ in the siddur.

Still to compare: the zimun, Lamnatzeach and Avarcha, Al HaNissim, Rachem,
Retzeh, and the English rubric wording.
