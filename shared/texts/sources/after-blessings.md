# Me'ein Shalosh and Borei Nefashot: sources

Section file: `shared/texts/after-blessings.tex`, used by both the birkon and Seder HaShulchan.

## Source

Transcribed from Yosef's Turkish siddur, pp. 349–350 (Yosef's decision to
include the after-blessings; the blessings before food are left out). This is
traditional liturgical wording.

## Editorial decisions

- The siddur's three-column tables (grain, wine, fruit) are set as indented
  alternatives, each with an English rubric, to fit the pocket page.
- The festival insertions are printed as full lines, one per festival.
- On Shabbat the siddur reads וְנַחֲמֵנוּ, noting וּרְצֵה וְהַחֲלִיצֵנוּ as an
  alternative; the alternative is given in the rubric.
- The English rubrics are our own wording, not the siddur's.
- Endings follow the base text's "-ach" forms (עַמָּךְ, עִירָךְ, כְּבוֹדָךְ,
  מִזְבָּחָךְ, הֵיכָלָךְ), matching Birkat HaMazon; the siddur has עַמֶּךָ … (Yosef).
- **Rosh HaShanah insertion:** the siddur has none, so it comes from the
  Sefaria base text (Siddur Edot HaMizrach, Torat Emet 357, public domain):
  וְזָכְרֵנוּ לְטוֹבָה בְּיוֹם הַזִּכָּרוֹן הַזֶּה, בְּיוֹם טוֹב מִקְרָא קֹדֶשׁ הַזֶּה (Yosef).
  The source's missing vowels (וְזכְרֵנוּ לטוֹבָה בּיוֹם) are supplied.

## Proofreading

Proofread by Yosef against the siddur (pp. 349–350): no corrections.
